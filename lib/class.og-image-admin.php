...
class Admin
{
	...
	public function __construct()
	{
		...
		add_action('admin_enqueue_scripts', function () {
			wp_enqueue_script(Admin::SCRIPT_STYLE_HANDLE, plugins_url('admin/admin.js', __DIR__), 'jquery', filemtime(dirname(__DIR__) . '/admin/admin.js'), true);
			wp_localize_script(Admin::SCRIPT_STYLE_HANDLE, 'bsi_settings', ['preview_url' => get_permalink() . Admin::BSI_IMAGE_NAME]);
		});
	}
	...
	public static function carbon_fields()
	{
		...
		$fields = array(
			Field::make('image', Admin::CF_DEFAULTS_PREFIX . 'image', 'The default OG:Image for any page/post/... that has no OG:Image defined.')->set_help_text(
				...
				'<br />' .
				$selection),
			Field::make('checkbox', Admin::CF_OPTION_PREFIX . 'image_use_thumbnail', 'Use the WordPress Featured image, if selected, before using the default image selected above.')->set_default_value(true),
			Field::make('text', Admin::CF_DEFAULTS_PREFIX . 'text', 'The default text to overlay if no other text or title can be found.')->set_help_text('This should be a generic text that is applicable to the entire website.'),
		);

		self::carbon_field__fonts($fields);
		$fields[] = self::carbon_field__position(Admin::CF_DEFAULTS_PREFIX . 'text_position', 'Default text position', true);

		self::carbon_field__color($fields, Admin::CF_DEFAULTS_PREFIX . 'color', 'Default Text color', '#FFFFFFFF');
		self::carbon_field__color($fields, Admin::CF_DEFAULTS_PREFIX . 'background_color', 'Default Text background color', '#66666666');

		if ('on' === Plugin::FEATURE_STROKE) {
			self::carbon_field__color($fields, Admin::CF_DEFAULTS_PREFIX . 'text_stroke_color', 'Default Text stroke color', '#00000000');
			...
			$fields[] = Field::make('text', Admin::CF_DEFAULTS_PREFIX . 'text_stroke', 'Default stroke width')->set_default_value(0);
		}
		if ('on' === Plugin::FEATURE_SHADOW) {
			self::carbon_field__color($fields, Admin::CF_DEFAULTS_PREFIX . 'text_shadow_color', 'Default Text shadow color', '#00000000');
			$fields[] = Field::make('text', Admin::CF_DEFAULTS_PREFIX . 'text_shadow_top', 'Shadow offset - vertical. Negative numbers to top, Positive numbers to bottom.')->set_default_value('-2');
			$fields[] = Field::make('text', Admin::CF_DEFAULTS_PREFIX . 'text_shadow_left', 'Shadow offset - horizontal. Negative numbers to left, Positive numbers to right.')->set_default_value('2');
		}
		if ('simple' === Plugin::FEATURE_SHADOW) {
			$fields[] = Field::make('checkbox', Admin::CF_DEFAULTS_PREFIX . 'text_shadow_enabled', 'Use a text shadow')->set_default_value('off');
		}
		$fields[] = Field::make('image', Admin::CF_OPTION_PREFIX . 'image_logo', 'Your logo')->set_help_text('For best results, use PNG with transparency at at least (!) 600 pixels wide and/or high. If you get "gritty" results, use higher values.');
		$fields[] = self::carbon_field__position(Admin::CF_DEFAULTS_PREFIX . 'logo_position', 'Default logo position', 'bottom-right');
		$fields[] = Field::make('text', Admin::CF_OPTION_PREFIX . 'image_logo_size', 'Size')->set_help_text('You can use a width (like 200), width and height (like 200x160) or a percentage (like 20%). This determines the bounding box, the logo aspect ratio will remain in tact.')->set_default_value('20%');
		...
		self::carbon_field__color($fields, Admin::CF_OPTION_PREFIX . 'color', 'Text color', get_site_option(Admin::DEFAULTS_PREFIX . 'color', '#FFFFFFFF'));
		$fields[] = self::carbon_field__position(Admin::CF_OPTION_PREFIX . 'text_position', 'Text position', get_site_option(Admin::DEFAULTS_PREFIX . 'text_position', 'bottom-right'));
		self::carbon_field__color($fields, Admin::CF_OPTION_PREFIX . 'background_color', 'Text background color', get_site_option(Admin::DEFAULTS_PREFIX . 'background_color', '#66666666'));
		if ('on' === Plugin::FEATURE_STROKE) {
			self::carbon_field__color($fields, Admin::CF_OPTION_PREFIX . 'text_stroke_color', 'Text stroke color', get_site_option(Admin::DEFAULTS_PREFIX . 'text_stroke_color', '#00000000'));
			$fields[count($fields) - 1]->set_help_text('Text-stroke in image-software is not a real stroke and will behave weirdly with text-transparency.');
			$fields[] = Field::make('text', Admin::CF_OPTION_PREFIX . 'text_stroke', 'Stroke width')->set_default_value(get_site_option(Admin::DEFAULTS_PREFIX . 'text_stroke', '0'));
		}
		if ('on' === Plugin::FEATURE_SHADOW) {
			self::carbon_field__color($fields, Admin::CF_OPTION_PREFIX . 'text_shadow_color', 'Text shadow color', get_site_option(Admin::DEFAULTS_PREFIX . 'text_shadow', '#00000000'));
			$fields[] = Field::make('text', Admin::CF_OPTION_PREFIX . 'text_shadow_top', 'Shadow offset - vertical. Negative numbers to top, Positive numbers to bottom.')->set_default_value(get_site_option(Admin::DEFAULTS_PREFIX . 'shadow_top', '-2'));
			$fields[] = Field::make('text', Admin::CF_OPTION_PREFIX . 'text_shadow_left', 'Shadow offset - horizontal. Negative numbers to left, Positive numbers to right.')->set_default_value(get_site_option(Admin::DEFAULTS_PREFIX . 'shadow_left', '2'));
		}
		if ('simple' === Plugin::FEATURE_SHADOW) {
			$fields[] = Field::make('checkbox', Admin::CF_OPTION_PREFIX . 'text_shadow_enabled', 'Use a text shadow')->set_default_value(get_site_option(Admin::DEFAULTS_PREFIX . 'shadow_enabled', 'off'));
		}

		$fields[] = Field::make('checkbox', Admin::CF_OPTION_PREFIX . 'logo_enabled', __('Use a logo on this image?'))->set_default_value('yes')->set_help_text('Uncheck if you do not wish a logo on this image, or choose a position below');
		$fields[] = self::carbon_field__position(Admin::CF_OPTION_PREFIX . 'logo_position', 'Logo position', get_site_option(Admin::DEFAULTS_PREFIX . 'logo_position', 'bottom-right'));
...
//		$fields[] = self::carbon_field__logo();
		...
		} elseif (!get_site_option(Admin::DEFAULTS_PREFIX . 'image')) {
	...
	private static function carbon_field__position($field_name, $field_label, $default = false)
	{
		...
		if (!$once) {
			...
			add_action('admin_footer', function () { ?>
				<style><?php print file_get_contents(__DIR__ . '/../css/positions.css'); ?></style><?php });
		}
	...
	{
		// google fonts
		$field = Field::make('text', Admin::CF_DEFAULTS_PREFIX . 'text__font', 'Font');
		...
		// TTF upload
		$fields[] = Field::make('file', Admin::CF_DEFAULTS_PREFIX . 'text__ttf_upload', 'Font upload')
	...
	{
		if (is_admin() && ($font_id = get_site_option(Admin::DEFAULTS_PREFIX . 'text__ttf_upload'))) {
			$font = get_attached_file($font_id);
			if (is_file($font)) {
				$instance = self::getInstance();
				update_site_option(Admin::DEFAULTS_PREFIX . 'text__ttf_upload', false);
				update_site_option(Admin::DEFAULTS_PREFIX . 'text__font', basename($font));
